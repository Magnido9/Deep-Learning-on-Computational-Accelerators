...
def mlp_experiment(
    ...
    model = BinaryClassifier(
        model=MLP(in_dim=2, dims=[*[width]*depth, 2], nonlins=[*['tanh']*(depth),'tanh']) #tanh
       
    )
    loss_fn = torch.nn.CrossEntropyLoss()
    lr = 0.035 # 0.00005 #0.03
    weight_decay = 0.01 # 0.000000001
    momentum = 0.85 # 0.00009 #0.8
    ...
    fit_result = trainer.fit(dl_train, dl_valid, num_epochs=n_epochs, print_every=0, verbose=False, plot=False)
...
def cnn_experiment(
    ...
    **kw,
    ...
    fit_res = None
    # ====== YOUR CODE: ======
    dl_train = torch.utils.data.DataLoader(ds_train, batch_size=bs_train,
                                           shuffle=False)
    dl_test = torch.utils.data.DataLoader(ds_test, batch_size=bs_train,
                                          shuffle=False)
    channels=[]
    for a in filters_per_layer:
        for i in range(layers_per_block):
            channels.append(a)
    x,_=ds_train[0]
    in_size=x.shape
    #    model=model_cls(pool_every=pool_every,hidden_dims=hidden_dims,in_size=in_size,channels=channels,out_classes=10,conv_params=dict(kernel_size=3,stride=1,padding=1),pooling_params=dict(kernel_size=2,stride=1))
    #    classifier=ArgMaxClassifier(model=model)
    n_size = ds_test[0][0].shape
    ...
    for filter in filters_per_layer:
        channels+=([filter] * layers_per_block)
    print(channels)
    model=model_cls(
        in_size, 10, channels=channels, pool_every=pool_every,
        hidden_dims=hidden_dims, conv_params=dict(kernel_size=3, stride=1, padding=1),
        pooling_params=dict(kernel_size=2,stride=1)
    )
    model=model.to(device)
    classifier = ArgMaxClassifier(
        model
    )
    loss=torch.nn.CrossEntropyLoss()
    optimizer=torch.optim.Adam(model.parameters())#,lr=lr,weight_decay=reg
    trainer=ClassifierTrainer(loss_fn=loss,optimizer=optimizer, model =classifier,device=device)
    fit_res=trainer.fit(dl_train=dl_train,dl_test=dl_test,num_epochs=epochs,print_every=1,early_stopping=early_stopping,checkpoints=checkpoints)
    # ========================
