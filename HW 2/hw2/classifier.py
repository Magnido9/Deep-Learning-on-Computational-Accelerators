...
class Classifier(nn.Module, ABC):
    ...
    def predict_proba_scores(self, z: Tensor) -> Tensor:
        """
        :param z:  tensor, e.g. calculated by this model.
        :returns: (N, C) i.e. C probability values between 0 and 1 for each of N
            samples.
        """
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        ret = torch.zeros(len(y_proba), dtype=torch.int32)
        i = 0
        #print(y_proba)
        for prob in y_proba:
            if prob[self.positive_class] > self.threshold:
                ret[i] = (1.0)
            i += 1
        return ret
        # =======================
...
def plot_decision_boundary_2d(
    ...
    x_min = torch.min(x[:, 0])
    x_max = torch.max(x[:, 0])
    x_steps = torch.linspace(x_min, x_max, steps=int((x_max - x_min).item()//dx))
    y_min = torch.min(x[:, 1])
    y_max = torch.max(x[:, 1])
    y_steps = torch.linspace(y_min, y_max, steps=int((y_max - y_min).item()//dx))
    x1_grid, x2_grid = torch.meshgrid(x_steps, y_steps, indexing='xy')
    x_g = x1_grid.resize(x1_grid.shape[0] * x1_grid.shape[1])
    y_g = x2_grid.resize(x2_grid.shape[0] * x2_grid.shape[1])
    ds = torch.vstack((x_g, y_g)).T
    y_hat = classifier.classify(ds).resize(x1_grid.shape[0],x2_grid.shape[1])
    # ========================
...
def select_roc_thresh(
    ...
    y_proba = y_proba.clone().detach().requires_grad_(False)
    #y_proba = torch.tensor(y_proba, requires_grad=False)
    y_proba = y_proba [:, 1]
    fpr, tpr, thresh = roc_curve(y,y_proba)
    J = tpr - fpr
    optimal_thresh_idx = torch.argmax(torch.tensor(J))
