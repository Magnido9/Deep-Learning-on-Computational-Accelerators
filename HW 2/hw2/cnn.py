...
class CNN(nn.Module):
    ...
    def H_out(self,Hin):
        return ((Hin+(2*self.conv_params["padding"])-(self.conv_params["kernel_size"]-1)-1)/self.conv_params["stride"])+1

    def H_out_pool(self,Hin):
        return ((Hin+2*self.conv_params["padding"]-1*(self.conv_params["kernel_size"]-1)-1)/self.conv_params["stride"])+1

    def W_out(self,Win):
        return ((Win+2*self.conv_params["padding"]-(self.conv_params["kernel_size"]-1)-1)/self.conv_params["stride"])+1

    def W_out_pool(self,Win):
        return ((Win+2*self.conv_params["padding"]-(self.conv_params["kernel_size"]-1)-1)/self.conv_params["stride"])+1
    ...
    def _make_feature_extractor(self):
        ...
        # ====== YOUR CODE: ======
        pooling = nn.MaxPool2d if self.pooling_type=='max' else nn.AvgPool2d
        ...
        for i in range(N):
            ...
            if(i!=0):
              a=self.channels[i-1]
            layers.append(nn.Conv2d(**{**(self.conv_params),'in_channels':a,'out_channels':self.channels[i]}))
            last_out=self.channels[i]
            in_h=self.H_out(in_h)
            in_w=self.W_out(in_w)
            layers.append(activation(**self.activation_params))
            if((i+1)%P==0):
                layers.append(pooling(**self.pooling_params))
                in_h=self.H_out_pool(in_h)
                in_w=self.W_out_pool(in_w)
        self.H=in_h
        self.W=in_w
    ...
    def _n_features(self) -> int:
        ...
        try:
            # ====== YOUR CODE: ======
            #in_channels, in_h, in_w= tuple(self.in_size)
            #v=self.feature_extractor(torch.zeros(in_channels))
            #print(v)
            t=self.feature_extractor(torch.zeros((1,*(self.in_size))))
    ...
    def _make_mlp(self):
        ...
        nonlin=([activation]*(len(self.hidden_dims)))+[nn.Identity()]
    ...
    def forward(self, x: Tensor):
        ...
        out: Tensor = None
        # ====== YOUR CODE: ======
        out=self.mlp(self.feature_extractor(x))
        # ========================
        return out
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(N-1):
            ...
            if i != 0:
                a=channels[i-1]
            layers.append(nn.Conv2d(**{'kernel_size':kernel_sizes[i],'in_channels':a,'out_channels':channels[i],'padding':((kernel_sizes[i]-1)//2),'bias':True}))
            last_out=channels[i]
            if (i<len(channels)-1):
                if dropout != 0:
        ...
        print(layers)
        print({'kernel_size':kernel_sizes[N-1],'in_channels':last_out,'out_channels':channels[N-1],'padding':((kernel_sizes[N-1]-1)//2)})
        layers.append(nn.Conv2d(**{'kernel_size':kernel_sizes[N-1],'in_channels':last_out,'out_channels':channels[N-1],'padding':((kernel_sizes[N-1]-1)//2)}))
        self.main_path = nn.Sequential(*layers)
        if in_channels != channels[-1]:
            self.shortcut_path = nn.Sequential(*[nn.Identity(),nn.Conv2d(**{'kernel_size':(1,1),'in_channels':in_channels,'out_channels':channels[N-1], 'bias':False})])
        else:
...
class ResidualBottleneckBlock(ResidualBlock):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #  architecture.
        # ====== YOUR CODE: ======
        param_channels = [inner_channels[0]]
        param_channels.extend(inner_channels)
        param_channels.append(in_out_channels)
        param_kernels = [1]
        param_kernels.extend(inner_kernel_sizes)
        param_kernels.append(1)
        super().__init__(in_channels=in_out_channels,channels=param_channels,kernel_sizes=param_kernels,**kwargs)
        # ========================
        ...
        for i in range(N):
            ...
            if((i+1)%P==0):
                if(i-P<0):
                    ...
                    tt=self.channels[(i-P)]
                in_c=self.channels[(i-(P-1)):i+1][-1]
                if(tt==self.channels[(i-(P-1)):i+1][-1] and self.bottleneck):
                    layers.append(ResidualBottleneckBlock(in_out_channels=tt,inner_channels=self.channels[(i-(P-2)):i] ,inner_kernel_sizes=[3]*(P-2),batchnorm=self.batchnorm,dropout=self.dropout,activation_type=self.activation_type,activation_params=self.activation_params))
                else:
                    layers.append(ResidualBlock(in_channels=tt,channels=self.channels[(i-(P-1)):i+1],kernel_sizes=[3]*(P),batchnorm=self.batchnorm,dropout=self.dropout,activation_type=self.activation_type,activation_params=self.activation_params))
                layers.append(pooling(**self.pooling_params))
        if(N%P>0):
            layers.append(ResidualBlock(in_channels=in_c,channels=self.channels[N-((N%P)):N+1],kernel_sizes=[3]*(N%P),batchnorm=self.batchnorm,dropout=self.dropout,activation_type=self.activation_type,activation_params=self.activation_params))
