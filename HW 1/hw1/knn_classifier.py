...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            values,indexes = (torch.topk(-dist_matrix[:,i],self.k))
            most_common,indexes = torch.mode(self.y_train[indexes])
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    # ====== YOUR CODE: ======
    # (a-b)^2 = a^2 + 2ab +
    x1p = torch.sum(x1**2, axis=1)
    x2p = torch.sum(x2**2,axis=1)
    x2st = 2*x1@(x2.T)
    x1p = torch.broadcast_to(x1p, x2st.T.shape)
    x2p = torch.broadcast_to(x2p, x2st.shape)
    dists = -x2st + x2p
    dists = dists.T + x1p

    # ========================
#RuntimeError: The expanded size of the tensor (45) must match the existing size (12) at non-singleton dimension 1.  Target sizes: [12, 45].  Tensor sizes: [12]

    return torch.sqrt(dists.T)
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    # ====== YOUR CODE: ======
    total = (y.shape[0])
    wrong = (torch.count_nonzero(y_pred-y).item())
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        ds_train_indexes=list(range(len(ds_train)))
        batch_size=int(len(ds_train)/num_folds)
        for j in range(num_folds):
            validate_set=DataLoader(dataset=ds_train,batch_size=batch_size,sampler=torch.utils.data.SubsetRandomSampler(
                ds_train_indexes[j*batch_size:(j+1)*batch_size]
            ))
            train_set=DataLoader(dataset=ds_train,batch_size=batch_size*(num_folds-j),sampler=torch.utils.data.SubsetRandomSampler(
                ds_train_indexes[:j*batch_size]+ds_train_indexes[(j+1)*batch_size:]
            ))
            model.train(train_set)
            x,y = next(iter(validate_set))
            predictions = model.predict(x)
            accuracies[i][j]=(accuracy(y,predictions))
        # ========================
