...
class LinearClassifier(object):
    ...
    def train(
        ...
        for epoch_idx in range(max_epochs):
            ...
            # ====== YOUR CODE: ======
            n=0
            for (train, result) in dl_train:
                n += 1
                pred, scores = self.predict(train)
                loss = loss_fn(train, result, scores, pred)
                grad = loss_fn.grad()
                self.weights = self.weights - grad * learn_rate
                average_loss += loss + weight_decay
                total_correct += self.evaluate_accuracy(result, pred)
            average_loss /= n
            total_correct /= n
    ...
    def weights_as_images(self, img_shape, has_bias=True):
        ...
        :param ie of each tensor image to create, i.e. (C,H,W).
...
def hyperparams():
    ...
    #  to pass.
    # ====== YOUR CODE: ======
    hp = dict(weight_std=0.042, learn_rate=0.069, weight_decay=0.11111111111111)
    # ========================
