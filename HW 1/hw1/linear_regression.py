...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        I = np.eye(X.shape[1])
        I[0, 0] = 0
        w_opt = np.matmul(np.linalg.inv(np.add(np.dot(X.T,X),I*self.reg_lambda*y.shape[0])),np.dot(X.T,y))
        # ========================
        self.weights_ = w_opt
        return self
...
def fit_predict_dataframe(
    ...
    y_pred = model.fit_predict(df[feature_names].values, df[target_name])
    # ========================
    return y_pred
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    top_n_corr = []
    top_n_features = df.corr(method='pearson').transpose()[target_feature].abs().sort_values()[-(n+1):].to_dict()
    top_n_features.pop(target_feature)

    top_n_features=list(top_n_features.keys())
    top_n_features.reverse()
    for (key,val) in df.corr(method='pearson').transpose()[target_feature].to_dict().items():
        if top_n_features.__contains__(key) and key!=target_feature:
...
def r2_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    r2 = 1 - (((np.sum((y - y_pred)**2)))/(np.sum((y - (np.sum(y)/y.shape[0]))**2)))
...
def cv_best_hyperparams(
    ...
    m = sklearn.model_selection.GridSearchCV(model, parameters, cv=k_folds, scoring= scorer)
    m.fit(X,y)
