...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            self.velocities[self.i] = self.momentum*self.velocities[self.i] - self.learn_rate*dp
            dp.add_(self.reg*p)
            p.add_(self.velocities[self.i])
            self.i += 1
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        self.ars = []
        for p,_ in params:
            self.ars.append(torch.zeros_like(p))
        self.i = 0
        # ========================

    def step(self):
        for (p, dp),r in zip(self.params,self.ars):
            self.i=0
            if dp is None:
                continue
