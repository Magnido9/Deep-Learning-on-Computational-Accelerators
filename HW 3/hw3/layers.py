...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        dx = 1 - ((torch.exp(x)-torch.exp(-x))**2)/((torch.exp(x)+torch.exp(-x))**2)
        dx = dx * dout
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        x_probs = torch.exp(x) / torch.sum(torch.exp(x),dim=1,keepdim=True)
        x_probs[range(N), y] -= 1
        dx = (x_probs * dout) / N
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            prob=torch.full(x.shape,self.p)
            bern=torch.bernoulli(prob)
            active_layers=bern/self.p
            self.grad_cache['active_layers']=active_layers
    ...
    def backward(self, dout):
        ...
        dx=dout
        if self.training_mode:
            dx=dout*self.grad_cache['active_layers']
        # ========================
