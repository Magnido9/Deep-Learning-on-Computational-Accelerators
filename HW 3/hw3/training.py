    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            test_result=self.test_epoch(dl_train,**kw)
            (train_losses,train_accuracy)=train_result
            (test_losses,test_accuracy)=test_result
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        self.optimizer.zero_grad()
        if self.prev==None:
            scores, self.prev = self.model(x)
        else:
            scores,self.prev=self.model(x,self.prev)
        scores=torch.transpose(scores,1,2).to(self.device)
