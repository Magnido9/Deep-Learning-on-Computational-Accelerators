...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    # ====== YOUR CODE: ======
    result=torch.torch.zeros(size=(len(text),len(char_to_idx.keys())),dtype=torch.int8)
    i=0
    for c in text:
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    #    torch.zeros(size=(int(((len(text)-1)/seq_len)),seq_len,len(char_to_idx.keys())))
    labels=torch.argmax(samples,dim=1).to(device)
        #torch.zeros(size=(int(((len(text)-1)/seq_len)),seq_len))
    i=0
    t= [text[i: i + seq_len] for i in range(0, len(text), seq_len)]
    samples=samples[:-1,:]
    labels=labels[1:]
    samples=torch.split(samples,seq_len,dim=0)
    labels=torch.split(labels,seq_len,dim=0)
    samples=samples[:int(len(text)/seq_len)]
    labels=labels[:int(len(text)/seq_len)]
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    # ====== YOUR CODE: ======
    t=None
    #,len(start_sequence),device
    samples=chars_to_onehot(start_sequence,char_to_idx).unsqueeze(0)
    samples=samples.float().to(device)
    n_chars-=len(start_sequence)
    with torch.no_grad():
        for i in range(n_chars):
            labels,t=model(samples,t)
            t1=torch.multinomial(hot_softmax(labels[0,-1],temperature=T),num_samples=1)
            out_text+=idx_to_char[t1.item()]
            samples=torch.unsqueeze(chars_to_onehot(out_text[-1],char_to_idx),0).float().to(device)
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        l=len(self.dataset)-len(self.dataset)%self.batch_size
        batch_amount=int(l/self.batch_size)
        idx = [0]*l
        z=0
        t=0
        for i in range(l):
            idx[(i%(batch_amount))*self.batch_size+z]=i
            t+=1
            if t%batch_amount==0:
                z+=1
        # ========================
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(n_layers):
            xz=nn.Linear(t,h_dim,bias=False)
            xr=nn.Linear(t,h_dim,bias=True)
            xg=nn.Linear(t,h_dim,bias=False)
            ...
            self.add_module('xz'+str(i),xz)
            self.add_module('xr'+str(i),xr)
            self.add_module('xg'+str(i),xg)
            self.add_module('hz'+str(i),hz)
            self.add_module('hr'+str(i),hr)
            self.add_module('hg'+str(i),hg)
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        # ====== YOUR CODE: ======
        layer_output=torch.zeros(layer_input.shape)
        for i in range(seq_len):
            x=input[:,i]
            first=True
            t=0
            for l in self.layer_params[:-1]:
                if first: # first layer
                    first=False
                    z=torch.sigmoid(l[0](x)+
                                    l[3](layer_states[t]))
                    r=torch.sigmoid(l[1](x)+l[4](layer_states[t]))
                    g=torch.tanh(l[2](x)+l[5](r*layer_states[t]))
                else:
                    z=torch.sigmoid(l[0](layer_states[t-1])+
                                    l[3](layer_states[t]))
                    r=torch.sigmoid(l[1](layer_states[t-1])+l[4](layer_states[t]))
                    g=torch.tanh(l[2](layer_states[t-1])+l[5](r*layer_states[t]))
                layer_states[t]=layer_states[t]*z+(1-z)*g
                if not first:
                    x=l[6]
                t+=1
